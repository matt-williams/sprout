...
Sproutlet* SproutletProxy::get_sproutlet(const std::string& alias)
{
  for (std::list<Sproutlet*>::iterator it = _sproutlets.begin();
       it != _sproutlets.end();
       ++it)
  {
    if ((*it)->service_name() == alias)
    {
      return (*it);
    }
    else
...
pj_status_t SproutletProxy::IntUASTsx::init(pjsip_tx_data* req, std::string alias, SAS::TrailId trail)
{
  BasicProxy::UASTsx::_trail = trail;
  _req = req;
  ...
  // TODO: Allow no default Sproutlet - forward immediately.
  Sproutlet* sproutlet = _sproutlet_proxy->get_sproutlet(alias);
  _root = new SproutletWrapper(_sproutlet_proxy,
                               this,
                               sproutlet,
                               alias,
