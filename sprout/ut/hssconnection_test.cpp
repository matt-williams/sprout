...
TEST_F(HssConnectionTest, SimpleAssociatedUris)
{
  ...
  std::string regstate;
  _hss.get_registration_data("pubid42", regstate, ifcs_map, uris);
...
TEST_F(HssConnectionTest, SimpleNotRegisteredGet)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.get_registration_data("pubid43", regstate, ifcs_map, uris);
...
TEST_F(HssConnectionTest, SimpleUnregistered)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.update_registration_state("pubid50", "", HSSConnection::CALL, regstate, ifcs_map, uris);
...
TEST_F(HssConnectionTest, SimpleNotRegisteredUpdate)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.update_registration_state("pubid50", "", HSSConnection::DEREG_ADMIN, regstate, ifcs_map, uris);
  EXPECT_EQ("NOT_REGISTERED", regstate);
}
...
TEST_F(HssConnectionTest, SimpleIfc)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.update_registration_state("pubid42", "", HSSConnection::REG, regstate, ifcs_map, uris);
...
TEST_F(HssConnectionTest, SimpleChargingAddrs)
{
  ...
  _hss.update_registration_state("pubid42", "", HSSConnection::REG, regstate, ifcs_map, uris, ccfs, ecfs);
...
TEST_F(HssConnectionTest, BadXML)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.update_registration_state("pubid42_malformed", "", HSSConnection::REG, regstate, ifcs_map, uris);
...
TEST_F(HssConnectionTest, BadXML_MissingRegistrationState)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.update_registration_state("missingelement1", "", HSSConnection::REG, regstate, ifcs_map, uris);
  EXPECT_TRUE(uris.empty());
...
TEST_F(HssConnectionTest, BadXML_MissingClearwaterRegData)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.update_registration_state("missingelement3", "", HSSConnection::REG, regstate, ifcs_map, uris);
  EXPECT_TRUE(uris.empty());
  EXPECT_TRUE(log.contains("Malformed Homestead XML"));
}
...
TEST_F(HssConnectionTest, BadXML_MissingIMSSubscription)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.update_registration_state("missingelement2", "", HSSConnection::REG, regstate, ifcs_map, uris);
  EXPECT_TRUE(uris.empty());
  EXPECT_TRUE(log.contains("Malformed HSS XML"));
}
...
TEST_F(HssConnectionTest, ServerFailure)
{
  ...
  std::map<std::string, Ifcs> ifcs_map;
  std::string regstate;
  _hss.update_registration_state("pubid44", "", HSSConnection::REG, regstate, ifcs_map, uris);
  EXPECT_EQ("", regstate);
  EXPECT_TRUE(uris.empty());
...
{
  Json::Value* actual;
  _hss.get_user_auth_status("privid69", "pubid44", "", "", actual);
...
{
  Json::Value* actual;
  _hss.get_user_auth_status("privid69", "pubid44", "domain", "REG", actual);
  ASSERT_TRUE(actual != NULL);
...
TEST_F(HssConnectionTest, CorruptAuth)
{
  CapturingTestLogger log;
  Json::Value* actual;
  _hss.get_user_auth_status("privid_corrupt", "pubid44", "", "", actual);
  ASSERT_TRUE(actual == NULL);
  EXPECT_TRUE(log.contains("Failed to parse Homestead response"));
  delete actual;
}
...
{
  Json::Value* actual;
  _hss.get_location_data("pubid44", false, "", actual);
  ASSERT_TRUE(actual != NULL);
  EXPECT_EQ("server-name", actual->get("scscf", "").asString());
  delete actual;
}
...
{
  Json::Value* actual;
  _hss.get_location_data("pubid44", false, "DEREG", actual);
  ASSERT_TRUE(actual != NULL);
  EXPECT_EQ("2001", actual->get("result-code", "").asString());
  delete actual;
}
...
{
  Json::Value* actual;
  _hss.get_location_data("pubid44", true, "CAPAB", actual);
  ASSERT_TRUE(actual != NULL);
  EXPECT_EQ("2001", actual->get("result-code", "").asString());
  delete actual;
}
...
{
  Json::Value* actual;
  HTTPCode rc = _hss.get_location_data("pubid45", false, "", actual);
  ASSERT_TRUE(actual == NULL);
  ASSERT_TRUE(rc == 404);
  delete actual;
}
